...
--[[
  ...
  for t = 1, T do
   ...
   if t==1 then
       ...
       local tmp = torch.mm(xt, Wxi) + torch.mm(prev_h, Whi)
       print("test whi sum = ", Whi:sum())
       print("t=1, it = ", tmp:sum())
       tmp = torch.mm(xt, Wxf) + torch.mm(prev_h, Whf)
       print("test whf sum = ", Whf:sum())
       print("t=1, ft = ", tmp:sum())
       tmp = torch.mm(xt, Wxo) + torch.mm(prev_h, Who)
       print("test who sum = ", Who:sum())
       print("t=1, ot = ", tmp:sum())
       tmp = torch.mm(xt, Wxg) + torch.mm(prev_h, Whg)
       print("test whg sum = ", Whg:sum())
       print("t=1, gt = ", tmp:sum())
    end
    local i = torch.sigmoid(torch.mm(xt, Wxi) + torch.mm(prev_h, Whi) + bi)
    local f = torch.sigmoid(torch.mm(xt, Wxf) + torch.mm(prev_h, Whf) + bf)
