  ...
  bool count_include_pad)
{
...
#if LOG_ENABLE
  fprintf(stderr,"  SpatialAveragePooling_MKLDNN__init_forward start, N=%d,inC=%d,inH=%d,inW=%d,kH=%d,kW=%d,dH=%d,dW=%d,padH=%d,padW=%d,outC=%d,outH=%d,outW=%d,ceilmode=%d\n",N,inC,inH,inW,kH,kW,dH,dW,padH,padW,outC,outH,outW,ceilmode );
#endif
  ...
  if(ceil_mode) {
    ...
    CHECK_ERR( MKLDNN_(dnnPoolingCreateForward)(&pool_fwd, attributes, dnnAlgorithmPoolingAvg, lt_user_input, kernelSize, kernelStride, pad, dnnBorderZeros), err );
    CHECK_ERR( MKLDNN_(dnnPoolingCreateBackward)(&pool_bwd, attributes, dnnAlgorithmPoolingAvg, lt_user_input, kernelSize, kernelStride, pad, dnnBorderZeros), err );
  } else {
    int pad[DIMENSION] = {-padW, -padH, -padW, -padH};
    CHECK_ERR( MKLDNN_(dnnPoolingCreateForward)(&pool_fwd, attributes, dnnAlgorithmPoolingAvg, lt_user_input, kernelSize, kernelStride, pad, dnnBorderZerosAsymm), err );
    CHECK_ERR( MKLDNN_(dnnPoolingCreateBackward)(&pool_bwd, attributes,dnnAlgorithmPoolingAvg, lt_user_input, kernelSize, kernelStride, pad, dnnBorderZerosAsymm), err );
  }
  ...
  if(size1 == size2) {
#if 0 
    fprintf(stderr,"MKLDNN_ AveragePooling forward ouput layout match OK\n");
#endif
  } else {
    if(!MKLDNN_(dnnLayoutCompare)(lt_user_output, lt_pool_forward_output)) {
      ...
      fprintf(stderr,"MKLDNN_ AveragePooling forward ouput layout match FAIL, size1 = %d, size2 = %d \n", size1, size2);
    }
...
#if LOG_ENABLE
  fprintf(stderr,"  SpatialAveragePooling_MKLDNN__init_forward end.\n" );
#endif
}
  ...
  THMKLTensor *gradInput)
{
...
#if LOG_ENABLE
  fprintf(stderr, "  SpatialAveragePooling_MKLDNN__init_backward start, N=%d, gradInC=%d, gradInH=%d, gradInW=%d, gradOutC=%d, gradOutH=%d, gradOutW=%d\n", \
  ...
  if(size1 == size2) {
#if 0
    fprintf(stderr,"MKLDNN_ AveragePooling bwddata input layout match OK\n");
#endif
  } else {
    if(!MKLDNN_(dnnLayoutCompare)(lt_user_gradInput, lt_pool_backward_gradInput)) {
      ...
      fprintf(stderr,"MKLDNN_ AveragePooling bwddata input layout match FAIL, size1 = %d, size2 = %d \n", size1, size2);
    }
...
#if LOG_ENABLE
  fprintf(stderr,"  SpatialAveragePooling_MKLDNN__init_backward end.\n" );
#endif
}
  ...
  bool count_include_pad)
{
  ...
  if(0 == initOK) {
    ...
    MKLNN_(SpatialAveragePooling_init_forward)(primitives, input, output, kH, kW, dH, dW, padH, padW, ceil_mode, count_include_pad);
  }
  ...
  fprintf(stderr,"  AveragePooling MKLDNN_ time forward = %.2f ms\n",duration );
#endif
}
  ...
  THMKLTensor *gradInput)
{
  ...
  dnnError_t err;
  if(initOK == 0) {
    MKLNN_(SpatialAveragePooling_init_backward)(primitives, input, gradOutput, gradInput);
  }
  dnnPrimitive_t pool_bwd = (dnnPrimitive_t) (primitives->tensor->storage->data[POOL_PRIM_BWD]);
  ...
  if(cvt_backward_gradOutput) {
    CHECK_ERR( MKLDNN_(dnnConversionExecute)(cvt_backward_gradOutput, gradOutput_data, buffer_backward_gradOutput), err );
  ...
  gettimeofday(&end,NULL);
  double duration = (end.tv_sec - start.tv_sec) * 1000 + (double)(end.tv_usec - start.tv_usec) /1000;
  fprintf(stderr,"  AveragePooling MKLDNN_ time backward = %.2f ms\n",duration );
#endif
